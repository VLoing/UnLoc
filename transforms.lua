...
-- Crop center at the bottom
function M.HorizCrop(size)
   return function(input)
      local w1 = math.ceil((input:size(3) - size)/2)
      local h1 = math.ceil((input:size(2) - size))
      return image.crop(input, w1, h1, w1 + size, h1 + size) --  horizontally center patch at the bottom
   end
end
